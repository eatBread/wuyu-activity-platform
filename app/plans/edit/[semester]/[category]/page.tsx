...
import { categoryMap, mockPlans } from '../../../../../lib/mockData'
...
export default function EditPlanPage({ params }: { params: { semester: string, category: string } }) {
  ...
  // 加载现有计划数据
  useEffect(() => {
    const currentPlans = mockPlans[params.semester as keyof typeof mockPlans]
    if (currentPlans && currentPlans.plans[params.category as keyof typeof currentPlans.plans]) {
      const plan = currentPlans.plans[params.category as keyof typeof currentPlans.plans]
      setFormData({
        semester: params.semester,
        category: params.category,
        title: plan.title,
        directions: plan.directions.map(dir => ({
          id: dir.id,
          name: dir.name,
          activities: dir.activities.map(act => ({
            id: act.id,
            name: act.name
          }))
        }))
  ...
  }, [params.semester, params.category])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const currentPlans = mockPlans[params.semester as keyof typeof mockPlans]
      if (currentPlans && currentPlans.plans[params.category as keyof typeof currentPlans.plans]) {
        (currentPlans.plans as any)[params.category] = {
  ...
  const canEditPlan = currentRole === 'GROUP_LEADER' || currentRole === 'PRINCIPAL'
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500 mb-6">只有教师组长和校长可以编辑活动计划</p>
          <Link
            href="/plans"
      ...
      {showSuccess && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-8 max-w-md mx-4 text-center">
            ...
            <h3 className="text-xl font-semibold text-gray-900 mb-2">计划更新成功！</h3>
            <p className="text-gray-600 mb-4">活动计划已成功更新，即将跳转到活动计划页面...</p>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <div className="flex justify-between items-start">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">编辑活动计划</h1>
              <p className="text-gray-600">修改现有的五育活动计划</p>
            </div>
            <div className="flex items-center space-x-3">
              ...
              >
                ...
                <span>{isSubmitting ? '更新中...' : '更新计划'}</span>
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  type="text"
                  value={formData.semester === '2024-2025-1' ? '2024-2025学年上学期' : '2024-2025学年下学期'}
                  disabled
